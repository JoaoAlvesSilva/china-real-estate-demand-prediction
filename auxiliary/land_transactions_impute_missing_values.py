...
land_transactions_nearby_sectors_path = (
    train_folder / "land_transactions_nearby_sectors.csv"
)
...
df_land_transactions["yr"] = df_land_transactions["month"].apply(func_str_to_yr)
df_land_transactions["m"] = df_land_transactions["month"].apply(func_str_to_month)

df_land_transactions_nearby_sectors["yr"] = df_land_transactions_nearby_sectors[
    "month"
].apply(func_str_to_yr)
df_land_transactions_nearby_sectors["m"] = df_land_transactions_nearby_sectors[
    "month"
].apply(func_str_to_month)
...
missing_combinations_land_transactions = (
    expected_combinations - current_combinations_land_transactions
)

missing_combinations_land_transactions_nearby_sectors = (
    expected_combinations - current_combinations_land_transactions_nearby_sectors
)

zero_columns_land_transactions = [
...
zero_columns_land_transactions_nearby_sectors = [
...
df_land_transactions_nearby_sectors = pd.concat(
    [df_land_transactions_nearby_sectors, df_missing_land_transactions_nearby_sectors],
    ignore_index=True,
)
...
df_land_transactions = df_land_transactions.sort_values(
    ["yr", "m", "sector_num"]
).reset_index(drop=True)
df_land_transactions_nearby_sectors = df_land_transactions_nearby_sectors.sort_values(
    ["yr", "m", "sector_num"]
).reset_index(drop=True)

df_land_transactions.to_csv(train_folder / "land_transactions_imputed.csv", index=False)
df_land_transactions_nearby_sectors.to_csv(
    train_folder / "land_transactions_nearby_sectors_imputed.csv", index=False
)
